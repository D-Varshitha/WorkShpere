...
const ManagerAttendance = () => {
  ...
  const fetchAttendance = async () => {
    try {
      setLoading(true);
      const res = await api.get('/manager/attendance/my');
      setAttendance(res.data || []);
    } catch (e) {
      console.error('Failed to fetch manager attendance', e);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-gray-500">Your monthly attendance log.</p>
      </div>
            ...
            return (
              <div key={day} className="relative group">
                ...
                >
                  ...
                  {status && <span className="text-[8px] uppercase font-black tracking-tighter mt-1">{status}</span>}
                </div>
              </div>
            );
          })}
